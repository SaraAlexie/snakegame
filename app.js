...
let hudHeight = 100; // Height for the HUD above the play area (adjusted to 100 for more space)
...
let playAreaGraphics; // Variable to hold the play area borders
...
function create() {
    ...
    // Add the background image (centered, keeping its original size)
    ...
    playAreaGraphics = this.add.graphics();
    playAreaGraphics.lineStyle(borderThickness, 0x000000); // Black border
    playAreaGraphics.strokeRect(
        playAreaPadding,
        hudHeight + playAreaPadding, // Adjusted for HUD height (increased)
        window.innerWidth - playAreaPadding * 2,
        window.innerHeight - hudHeight - playAreaPadding * 2 // Adjusted for HUD height (increased)
    );
    playAreaGraphics.setVisible(false); // Hide borders initially
...
function startGame() {
    ...
    // Show snake, food, score, highscore, and play area borders
    snake.setVisible(true);
    food.setVisible(true);
    scoreText.setVisible(true);
    highscoreText.setVisible(true); // Show highscore when game starts
    playAreaGraphics.setVisible(true); // Show the play area borders
}
...
function handleOverlap(head, food) {
    ...
    do {
        food.setPosition(
            ...
            randomPosition(
                window.innerHeight - hudHeight - playAreaPadding * 2
            ) + hudHeight // Ensure it spawns below HUD
        );
    ...
    // Increase snake speed by 1 for every fruit eaten
    snakeSpeed = Math.max(snakeSpeed - 1, 50); // Increase speed, with a minimum limit of 50
...
function randomPosition(max) {
    ...
    return (
        Math.floor((Math.random() * (maxPosition - min)) / gridSize) *
            gridSize +
        min
    );
}
...
function resizeGame(scene) {
    ...
    backgroundImage.setPosition(width / 2, height / 2); // Keep it centered

    // Ensure play area borders are adjusted during resize
    playAreaGraphics.clear();
    playAreaGraphics.lineStyle(borderThickness, 0x000000);
    playAreaGraphics.strokeRect(
        playAreaPadding,
        hudHeight + playAreaPadding,
        width - playAreaPadding * 2,
        height - hudHeight - playAreaPadding * 2
    );
}
